...
# restart-skip: if this config already finished (model + done marker present), don't redo it
if os.path.exists(OUT + ".npy") and os.path.exists(OUT + "_done.txt"):
    print(f"SKIP {OUT}: already finished (found {OUT}.npy + _done.txt)")
    raise SystemExit(0)
...
def save_model(wlist, blist):
    """Write the mlp-compatible 6-element .npy atomically. Same format/scale as the final
    save, so a mid-training checkpoint IS a usable model if the run dies before EPOCHS."""
    Wc = [tf.constant(w) for w in wlist]; Bc = [tf.constant(b) for b in blist]
    ns_x5 = tl.uniform_norm_sample(200000, np.random.RandomState(3))[1]
    sc = 1.0 / float(np.mean(forward(tl.features_from_orig(ns_x5), Wc, Bc).numpy()))
    tmp = OUT + ".tmp.npy"
    np.save(tmp, np.array([sc, RANGES, wlist, blist, "swish", "softplus"], dtype=object))
    os.replace(tmp, OUT + ".npy")   # atomic rename -> never leaves a half-written file
...
for e in range(EPOCHS):
    ...
    if e % CHECK == 0:
        ...
        if e % (CHECK * 20) == 0:
            ...
            if bestW is not None:   # crash-safe checkpoint of the best-so-far weights
                save_model(bestW, bestB)
                with open(OUT + "_progress.txt", "w") as fh:
                    fh.write(f"epoch {e} best_val {best:.1f} @ {best_ep}\n")
...
# final save (mlp-compatible 6-element format) + done marker so a restart skips this config
save_model(bestW, bestB)
with open(OUT + "_done.txt", "w") as fh:
    fh.write(f"done best_val {best:.1f} @ epoch {best_ep}\n")
