"""
4-body B->K*K* acceptance -- SWISH variant (ablation vs the sigmoid baseline).

Same as train_acc_5d.py but the MLP uses a non-saturating hidden activation (swish by
default) with a strictly-positive output (softplus).  Motivation: the original sigmoid
MLP saturates -> slow convergence + can't sharpen the mass-window edge.  Non-saturating
hidden activations keep gradients alive (faster) and represent sharper features, while
softplus keeps the density > 0.  Leaves the sigmoid pipeline (train_acc_5d.py) untouched.

Saves a self-describing 6-element model: [scale, ranges, weights, biases, hidden, out],
so the eval tools pick up the right activations automatically.

Env: ACC_ACT (hidden, default swish), ACC_OUT (output, default softplus) + all the
ACC_* knobs of train_acc_5d.py (LR, LR_FINAL, LAMBDA2, NORM_SIZE, NTRAIN, MAX_EPOCHS,
VAL_FRAC, CHECK_EVERY, PATIENCE, SEED, FLATTEN).
"""
import os
import math
import numpy as np
...
OUTFILE    = os.environ.get("ACC_OUTFILE", os.path.join(HERE, "acc_5d_swish"))
N_HIDDEN   = [32, 64, 32, 8]
HIDDEN_ACT = _s("ACC_ACT", "swish")           # non-saturating hidden activation
OUT_ACT    = _s("ACC_OUT", "softplus")        # strictly-positive output
...
def main():
    atfi.set_seed(SEED)
    np.random.seed(SEED)
    print(f"MLP: hidden={HIDDEN_ACT}, output={OUT_ACT}, layers={N_HIDDEN}")
    ...
    # ---- training loop (best-tracking; early-stop if PATIENCE small) ----
